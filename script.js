  ...
  let game = {
    ...
    checkInputs: function () {
      if (
        this.$P1Name.val() != '' &&
        this.$P2Name.val() != '' &&
        (this.$choice.filter(':checked').val() === '1' ||
          this.$choice.filter(':checked').val() === '2')
      ) {
        InitialForm.style.display = 'none';
        this.displayPlayer();
      } else {
        alert('You need to fill out both names and select a game option');
      }
    },
        ...
        for (let i = 1; i <= 9; i++) {
          if (this.squares[i] != '' && this.gameState.hvh === 1) {
            if (this.squares[i] === 'X') {
              ...
              setTimeout(function () {
                document.getElementById(i).classList.add('pre-animation1');
              }, 50);
            } else if (this.squares[i] === 'O') {
              document.getElementById(i).innerHTML = this.squares[i];
              setTimeout(function () {
                document.getElementById(i).classList.add('pre-animation2');
              }, 50);
            }
          }
    ...
    easyAI: function () {
      let AImove =
        this.availableSpaces[
          Math.floor(Math.random() * this.availableSpaces.length)
        ];
    ...
    displayPlayer: function () {
      if (
        ...
        document.getElementById(
          'displayTurn'
        ).innerHTML = `Current Player: ${this.Player1.name}`;
      } else if (
        this.turns.length == 1 ||
        this.turns.length == 3 ||
        this.turns.length == 5 ||
        ...
        document.getElementById(
          'displayTurn'
        ).innerHTML = `Current Player: ${this.Player2.name}`;
      }
    },
    ...
    checkConditionsX: function () {
      ...
      if (
        ...
        (this.squares[3] == X &&
          this.squares[5] == X &&
          this.squares[7] == X) ||
        (this.squares[1] == X && this.squares[5] == X && this.squares[9] == X)
      ) {
        modal.style.display = 'block';
        if (this.gameState.hvh === 1) {
          document.getElementById(
            'Player1'
          ).innerHTML = `Good Game! ${this.Player1.name} won the game!!`;
        } else {
          document.getElementById('Player1').innerHTML = `Good Game! You won!!`;
        }
    ...
    checkConditionsO: function () {
      ...
      if (
        ...
        if (this.gameState.hvh === 1) {
          ...
          document.getElementById(
            'Player2'
          ).innerHTML = `Good Game! ${this.Player2.name} won the game!!`;
          this.turns.push(null, null, null, null);
        } else {
          modal2.style.display = 'block';
          document.getElementById(
            'Player2'
          ).innerHTML = `Close one, the computer player, ${this.Player2.name}, won the game!!`;
          this.turns.push(null, null, null, null);
        }
      }
    },
    ...
    impossibleAI: function () {
      ...
      if (this.turns.length == 1) {
        ...
        if (
          ...
          this.chooseAI(1);
        if (this.squares[6] == 'X' || this.squares[8] == 'X') this.chooseAI(9);
      }
    },
